...
const getAllAccessLog = async (req, res) => {
  try {
    ...
    let accessLogs = await AccessLog.find({})
      .populate({
        path: "user",
      })
      .populate({
    ...
    // console.log("accessLogsBefore", accessLogs);
    if (type) {
      //   console.log("accessLogsBefore", accessLogs);

      accessLogs = accessLogs.filter(
        (log) => log.device && log.device.type === type
      );
    }
    // console.log("accessLogsAfter", accessLogs);
...
const createAccessLog = async (req, res) => {
  try {
    const { performance, status, userId, deviceId, mode } = req.body;
