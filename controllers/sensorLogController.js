...
const getAllSensorLog = async (req, res) => {
  try {
    ...
    if (type)
      sensorLogs = sensorLogs.filter(
        (log) => log.sensor && log.sensor.type === type
      );
...
const createSensorLog = async (req, res) => {
  try {
    const { temperature, humidity, CO2, CO, sensorId, status } = req.body;
...
const editSensorLog = async (req, res) => {
  try {
    const { _id, temperature, humidity, CO2, CO, sensorId, status } = req.body;
    ...
    sensorLog.CO2 = CO2 || sensorLog.CO2;
    sensorLog.CO = CO || sensorLog.CO;
